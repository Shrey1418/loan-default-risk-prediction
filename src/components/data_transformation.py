...
class DataTransformation:
    ...
    def clean_data(self, df):
        try:
            df["EDUCATION"] = df["EDUCATION"].replace([0, 5, 6], "other").astype(str)
            df["MARRIAGE"] = df["MARRIAGE"].replace(0, "other").astype(str)
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            logging.info(
                f"SMOTE applied — before: {X_train_transformed.shape}, after: {X_train_resampled.shape}"
            )

            save_object(self.preprocessor_obj_path, preprocessor)
            logging.info(f"Preprocessor object saved at {self.preprocessor_obj_path}")
